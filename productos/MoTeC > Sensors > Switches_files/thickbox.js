...
function TB_init(){
	$("img.lightbox").each(function(){
		// TODO: use a regular expression for more precise matching
		src = this.src.substr(0, this.src.indexOf('&'));
		$(this).wrap('<a class="thickbox" target="image" href="'+src+'" title="'+this.title+'"></a>');
	});
...
function TB_show(caption, url) {//function called when the user clicks on a thickbox link
	try {
		...
		// match the extension, not the full url
		extension = url.substr(url.lastIndexOf('.'), url.length).toLowerCase();
		var urlType = extension.match(urlString);
		...
		if(urlType==null||urlType=='.htm'||urlType=='.html'||urlType=='.php'||urlType=='.asp'||urlType=='.aspx'||urlType=='.jsp'||urlType=='.jst'||urlType=='.rb'||urlType=='.txt'||urlType=='.cfm' || (url.indexOf('TB_inline') != -1)){//code to show html pages
			...
			$("#TB_closeWindowButton").click(TB_remove);
				...
				if(url.indexOf('TB_inline') != -1){
					...
					TB_WIDTH = 800;
					TB_HEIGHT = 500;
					$("#TB_ajaxContent").html('<iframe src="'+url+'" />');
					TB_position();
					$("#TB_load").remove();
					$("#TB_window").fadeIn("fast");
				}
